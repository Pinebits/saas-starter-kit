...
const CreateTenant = ({ visible, setVisible }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        router.push(`/tenants/${json.data.slug}/settings`);
      } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">{t('create-tenant')}</h1>
      
      <Card>
        <Card.Header>
          <Card.Title>{t('tenant-details')}</Card.Title>
          <Card.Description>{t('members-of-a-tenant')}</Card.Description>
        </Card.Header>
        <Card.Body>
